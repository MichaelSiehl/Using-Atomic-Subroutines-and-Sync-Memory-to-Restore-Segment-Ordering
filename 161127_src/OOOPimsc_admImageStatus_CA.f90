...
private :: OOOPimscSAElement_atomic_increment_intImageSyncMemoryCount_CA
!***
...
!
!********** 161126: track the segment execution on every image:
! private:
...
integer(OOOGglob_kint) :: intSegment ! only to produce a test-output
  !
                                                                call OOOGglob_subSetProcedures &
                                                            ("OOOPimscSAElement_atomic_increment_intImageSyncMemoryCount_CA")
  !
  ...
  call atomic_add(Object_CA % mA_atomic_intImageSyncMemoryCount(this_image()), 1) ! Fortran 2015 syntax
!
! test:
call atomic_ref(intSegment, Object_CA % mA_atomic_intImageSyncMemoryCount (this_image())) ! only to produce a test-output
